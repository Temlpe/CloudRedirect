...
static void TryAutoUpdateDll() {
    ...
    HINTERNET hReq = WinHttpOpenRequest(hConn, L"GET",
        L"/repos/pvzcxw/CloudRedirect/releases",
    ...
    if (configFile) {
        ...
        if (!providerName.empty() && providerName != "local") {
            ...
            if (provider) {
                ...
                if (providerName == "workshop") {
                    // The workshop provider reads its settings (sync_path,
                    // workshop_*) straight from config.json.
                    initPath = configStr;
                    auto* workshop = dynamic_cast<WorkshopProvider*>(provider.get());
                    if (workshop) {
                        workshop->SetNotifyFn([](const std::string& message) {
                            NotifyUser(CR_NOTIFY_WARN, "CloudRedirect - Steam Workshop",
                                       message.c_str());
                        });
                    }
                }
