...
std::unique_ptr<ICloudProvider> CreateCloudProvider(const std::string& name) {
    ...
    // Steam Workshop: local mirror + workshop_sync_tool.exe background sync.
    // No OAuth; "authentication" is implicit in the logged-in Steam user.
    if (lower == "workshop") {
        return std::make_unique<WorkshopProvider>();
    }
...
std::string ResolveProviderTokenPath(const std::string& configDir,
    ...
    // The workshop provider reads its settings straight from config.json
    // (sync_path + workshop_* keys).
    if (provider == "workshop") return configDir + "config.json";
#ifdef _WIN32
