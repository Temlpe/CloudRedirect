...
public partial class CloudProviderPage : Page
{
    ...
    // Suppresses workshop setting handlers during programmatic load.
    ...
    private void UpdateProviderUI()
    {
        ...
        bool isWorkshop = tag == "workshop";
        bool needsPathRow = needsOAuth || isFolder || isWorkshop; // R2/S3 hide the path row

        // Token path row: visible for OAuth/folder/workshop, hidden for R2.
        ...
        // Steam Workshop settings panel.
        WorkshopPanel.Visibility = isWorkshop ? Visibility.Visible : Visibility.Collapsed;
        ...
        {
            PathLabel.Text = S.Get("CloudProvider_WorkshopSyncFolderPath");
            TokenPathBox.PlaceholderText = S.Get("CloudProvider_SyncFolderPlaceholder");
            PathHint.Text = S.Get("CloudProvider_WorkshopSyncFolderHint");
    ...
    private void BrowseToken_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (provider == "folder" || provider == "workshop")
        {
    ...
    private async Task<bool> SaveConfigSilent()
    {
        ...
        // Register this provider's path (skip folder/local/workshop — they use sync_path).
        if (provider != "folder" && provider != "local" && provider != "workshop" &&
        ...
        try
        {
            Services.ConfigHelper.SaveConfig(configPath,
                new[] { "provider", "sync_path", "token_path", "token_paths",
                        "workshop_appid", "workshop_visibility", "workshop_tool_path" },
                writer =>
                {
                    ...
                    {
                        if (int.TryParse(WorkshopAppIdBox.Text?.Trim(), out int appid) && appid > 0)
                            writer.WriteNumber("workshop_appid", appid);
                        if (WorkshopVisibilityCombo.SelectedItem is ComboBoxItem visItem &&
                            int.TryParse(visItem.Tag as string, out int vis))
                            writer.WriteNumber("workshop_visibility", vis);
                        var tool = Services.EmbeddedWorkshopTool.EnsureExtracted();
                        if (!string.IsNullOrEmpty(tool))
                            writer.WriteString("workshop_tool_path", tool);
                    }
    ...
    private void UpdateAuthStatus(Services.TokenStatus? preCheckedStatus = null)
    {
        ...
        {
            var folderPath = TokenPathBox.Text?.Trim();
            string? tool = Services.EmbeddedWorkshopTool.EnsureExtracted();
            if (string.IsNullOrEmpty(folderPath))
            {
                AuthStatus.Text = S.Get("CloudProvider_WorkshopNoFolder");
                AuthIcon.Symbol = Wpf.Ui.Controls.SymbolRegular.ShieldKeyhole24;
            }
            else if (!Directory.Exists(folderPath))
            {
            ...
            {
                AuthStatus.Text = S.Get("CloudProvider_WorkshopToolMissing");
                AuthIcon.Symbol = Wpf.Ui.Controls.SymbolRegular.ShieldDismiss24;
            }
            else
            {
                AuthStatus.Text = S.Get("CloudProvider_WorkshopAuthOk");
                AuthIcon.Symbol = Wpf.Ui.Controls.SymbolRegular.ShieldCheckmark24;
            }
            return;
        }
    ...
    /// <summary>
    /// Populate the workshop AppID / visibility controls from config.json.
    /// Synchronous by design: called from UpdateProviderUI (user gesture or
    /// the _loading-guarded Loaded path) with a tiny file.
    /// </summary>
    private void LoadWorkshopSettings()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                {
                    using var doc = JsonDocument.Parse(File.ReadAllText(path));
                    if (doc.RootElement.TryGetProperty("workshop_appid", out var a) && a.TryGetInt32(out var ai))
                        appid = ai;
                    if (doc.RootElement.TryGetProperty("workshop_visibility", out var v) && v.TryGetInt32(out var vi))
                        visibility = vi;
                }
            }
            catch { }

            WorkshopAppIdBox.Text = appid.ToString();
            foreach (ComboBoxItem item in WorkshopVisibilityCombo.Items)
            {
                if (item.Tag as string == visibility.ToString())
                {
                    WorkshopVisibilityCombo.SelectedItem = item;
                    break;
                }
            }
        }
    ...
    private void WorkshopSettings_LostFocus(object sender, RoutedEventArgs e)
    {
        if (_loading || _workshopLoading) return;
        _ = SaveConfigSilent();
    }

    private void WorkshopVisibility_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (_loading || _workshopLoading) return;
        _ = SaveConfigSilent();
    }

    private async void WorkshopOpenPage_Click(object sender, RoutedEventArgs e)
    {
        int appid = 480;
        if (!int.TryParse(WorkshopAppIdBox.Text?.Trim(), out appid) || appid <= 0)
        {
            await Services.Dialog.ShowWarningAsync(S.Get("CloudProvider_WorkshopPanelTitle"),
                S.Get("CloudProvider_WorkshopAppId"));
            return;
        }
        var url = $"https://steamcommunity.com/workshop/browse/?appid={appid}&browsesort=myworkshop";
        try
        {
