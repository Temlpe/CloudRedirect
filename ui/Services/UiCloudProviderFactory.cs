...
internal static class UiCloudProviderFactory
{
    public static IUiCloudProvider? TryResolve(CloudConfig? config, Action<string>? log)
    {
        ...
        return config.Provider switch
        {
            ...
            // The workshop mirror uses the same on-disk layout as the folder
            // provider, so cleanup/backup UI features work unchanged.
            "workshop" => new FolderUiCloudProvider(log, config.SyncPath!),
            _          => null,
        };
    }
}
